...
maximize profit:
	sum {j in weeks} (
		sum {i in cuts} (sold_fresh[i, j, "primary"] + sold_store[i, j, "primary"] * discount) * primary_price[i]
		+ (sold_fresh["trim85", j, "secondary"] + sold_store["trim85", j, "secondary"] * discount) * ground_price
		+ (sold_fresh["bone", j, "secondary"] + sold_fresh["trim65", j, "secondary"] 
			+ (sold_store["bone", j, "secondary"] + sold_store["trim65", j, "secondary"]) * discount) * processor_price
		+ sold_hotdog[j] * hotdog_price
		- (singles[j] / 45 * purchase_cost
...
subject to Balance_week_j {i in cuts, j in weeks}:
	storage[i, j - 1] + convert_input[i, j] + singles[j] / 45 * yield[i] 
...
subject to supply_fresh_cut {i in cuts, j in weeks}:
	sum {k in types} sold_fresh[i, j, k] <= singles[j] / 45 * yield[i];
...
subject to availability {j in weeks}:
	ground_minimum * ground_binary[j] <= convert_input["trim85", j] + singles[j] / 45 * yield["trim85"]
				+ storage["trim85", j - 1] 
...
# Constraint 10 - load calculation
subject to lb {j in weeks}:
	load[j] >= singles[j] / 45;

subject to ub {j in weeks}:
	load[j] <= singles[j] / 45 + 1;
